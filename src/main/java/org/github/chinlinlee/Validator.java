...
import org.github.chinlinlee.rest.IgResponse;
...
public class Validator {
    ...
    public OperationOutcome validate(byte[] resource, List<String> profiles) {
        ...
        try {
            ...
            System.out.println("The validationEngine error");
     ...
     * Load a profile into the validator.
     *
     * @param source the json/xml string of profile to be loaded
     */
    public void loadProfile(String source) throws FHIRException, IOException {
        if (hl7Validator.getContext().hasResource(StructureDefinition.class, source)) {
            System.out.print("profile already in validator, skip loading");
            return;
        }
        if (hl7Validator.getContext().hasResource(ImplementationGuide.class, source)) {
            System.out.print("profile already in validator, skip loading");
            return;
        }

        byte[] profileSource = source.getBytes(StandardCharsets.UTF_8);
        this.loadProfile(profileSource);
